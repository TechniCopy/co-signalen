...
// ─── PROGRESS BAR (missie 1: 2 rondes, missie 2: 3 rondes + 5 hartjes) ───

const RONDES_PER_MISSIE = { 1: 2, 2: 3 };
...
export function MCControle({ pool, addScore, loseLife, onComplete, lastRound = false, label = "Controlevraag", nextLabel, noteer }) {
  ...
  // pogingen; bij de eerste fout alleen een hint, zodat de cursist zelf
        ...
        <div className="mt-2">
          {isCorrect ? (
            ...
            <div
              ...
              style={{ backgroundColor: "#FDF1E3", borderColor: "#D9A62E", color: C.brownText }}
            >
...
// leermomenten = de vaste kern van het leerdoel (altijd tonen)
// aandacht = de punten waar deze speler zelf de mist in ging (alleen tonen als er zijn)
export function EndScreen({ score, maxScore, lives, text, leermomenten = [], aandacht = [], onRestart, onExit, exitLabel = "Terug naar het menu" }) {
    ...
    <div className="flex-1 flex flex-col items-center justify-center gap-6 p-8">
      ...
      {leermomenten.length > 0 && (
        <div className="border-2 rounded-2xl p-5 max-w-lg w-full" style={{ backgroundColor: C.bgCard, borderColor: C.beigeMid }}>
          <div className="text-xs font-bold uppercase tracking-widest mb-2.5" style={{ color: C.olive }}>
          ...
          {aandacht.length > 0 && (
            <>
              <div className="text-xs font-bold uppercase tracking-widest mt-4 mb-2.5" style={{ color: C.amber }}>
